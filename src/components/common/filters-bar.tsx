...
export interface FiltersBarConfig {
  ...
  /**
   * Quick-filter de un click bajo la búsqueda (segmented control shadcn Tabs)
   * para los estados ACCIONABLES de la vista — máx ~4 opciones para que quepa
   * en mobile. Escribe el mismo param nuqs shallow que el resto de filtros;
   * el listado completo de estados sigue disponible en el popover via
   * `config.status` (ambos leen/escriben el mismo param, así que siempre
   * muestran el valor real). `counts` (value → número) dibuja un badge con la
   * carga accionable del estado; se omiten los valores sin count o con 0.
   */
  tabs?: { paramKey: string; options: SelectOption[]; counts?: Partial<Record<string, number>> };
  combobox?: {
    label: string;
    paramKey: string;
...
export function FiltersBar({ parsers, config, defaults, customContent, labels = {} }: FiltersBarProps) {
  ...
  const active = Object.keys(defaults).some((k) => (params[k] ?? '') !== (defaults[k] ?? ''));
    ...
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-1">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant={active ? 'default' : 'outline'} size="sm" className="h-8 gap-1.5 px-2 md:h-10 md:gap-1 md:px-3">
            ...
            <div className="space-y-1">
              ...
              {config.selects?.map((sel) => (
                <div key={sel.paramKey} className="space-y-1">
                  ...
                  <Select value={getParam(sel.paramKey) || 'ALL'} onValueChange={(value) => setParam(sel.paramKey, value)}>
                    <SelectTrigger className="h-8 text-xs md:h-9 md:text-sm">
                      <SelectValue placeholder="Todos" />
                    </SelectTrigger>
                    <SelectContent>
      ...
      {config.tabs &&
        (() => {
          ...
          return (
            <Tabs
              value={getParam(tabs.paramKey) || 'ALL'}
              onValueChange={(value) => setParam(tabs.paramKey, value)}
              className="custom-scrollbar w-full overflow-x-auto"
            >
              <TabsList className="h-8 w-full">
                {tabs.options.map((opt) => {
                  const count = tabs.counts?.[opt.value];
                  return (
                    <TabsTrigger key={opt.value} value={opt.value} className="px-2.5 text-xs md:px-3">
                      {opt.label}
                      {count !== undefined && count > 0 && (
                        <span className="flex h-4 min-w-4 items-center justify-center rounded-full bg-primary/15 px-1 text-[10px] font-semibold leading-none text-primary tabular-nums">
                          {count > 99 ? '99+' : count}
                        </span>
                      )}
                    </TabsTrigger>
