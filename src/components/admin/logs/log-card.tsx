...
export function LogCard({ log, isExpanded, isHighlighted, onToggle }: LogCardProps) {
  ...
  const activeBg = log.level === 'error' ? 'bg-red-500/10 dark:bg-red-500/15' : undefined;
  ...
  const title = (
    <div className="flex items-center gap-1.5">
      ...
      {log.flow && <span className="text-muted-foreground text-xs">/{log.flow}</span>}
      {log.action && <span className="bg-muted rounded px-1.5 py-0.5 font-mono text-[10px]">{log.action}</span>}
    </div>
  );
  ...
  return (
    <RegistryCard
      ...
      topRightContent={
        log.userName ? (
          <span className="text-muted-foreground text-xs font-medium">{log.userName}</span>
        ) : log.userId ? (
    ...
    >
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          {log.userName && (
            <div>
              <h4 className="text-muted-foreground mb-1 text-xs font-medium uppercase tracking-wider">Usuario</h4>
              <p className="bg-muted rounded px-2 py-1 text-xs font-medium">{log.userName}</p>
            </div>
          )}
          {log.userId && (
            <div>
          ...
          {log.ip && (
            <div>
              <h4 className="text-muted-foreground mb-1 text-xs font-medium uppercase tracking-wider">IP</h4>
              <p className="bg-muted rounded px-2 py-1 font-mono text-xs">{log.ip}</p>
            </div>
          )}
        </div>
      </div>
