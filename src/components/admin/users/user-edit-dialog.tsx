...
interface UserEditDialogProps {
  user: User | null;
  onClose: () => void;
  /** Invalidar la lista de usuarios tras una mutación exitosa. */
  onChanged: () => void;
  /** Abrir el AlertDialog de desvinculación de Telegram (vive en el parent). */
  onUnlinkRequest: (user: User) => void;
}

export function UserEditDialog({ user: editUser, onClose, onChanged, onUnlinkRequest }: UserEditDialogProps) {
  ...
  // Inicializar el form al abrir con un usuario nuevo
  useEffect(() => {
    if (!editUser) return;
    startTransition(() => {
      setEditForm({
        creditLimit: editUser.creditLimit.toString(),
        minAmount: editUser.minAmountPreference?.toString() || '',
        maxAmount: editUser.maxAmountPreference?.toString() || '',
        allowSearchPreferences: editUser.allowSearchPreferences,
        allowBuyRateAdjustment: editUser.allowBuyRateAdjustment,
      });
    });
  }, [editUser]);
  ...
  return (
    <EditRoot open={!!editUser} onOpenChange={(open) => !open && onClose()}>
          ...
          <div className="mt-4 border-t pt-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {editUser?.telegramUser && (
                <Button variant="outline" size="sm" className="text-destructive" onClick={() => editUser && onUnlinkRequest(editUser)}>
