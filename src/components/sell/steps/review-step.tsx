  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 items-start">
      ...
      <Card className="md:col-span-4 border-border bg-card/50 backdrop-blur-sm p-4 md:p-6 space-y-4 md:space-y-6 flex flex-col h-auto md:h-full sticky top-0 z-20">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-foreground mb-0.5 md:mb-1">Summary</h2>
        ...
        <div className="space-y-3 md:space-y-4">
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 md:p-4 space-y-2">
            <div className="flex justify-between items-center text-sm md:text-xs">
              ...
              <Badge variant="outline" className="bg-primary/10 text-primary border-primary/20 text-sm px-1.5 py-0">
                {sellRate * 100}% Rate
              </Badge>
            </div>
            <div className="text-3xl md:text-4xl font-black text-primary">${totalToReceive.toFixed(2)}</div>
            <p className="text-sm text-muted-foreground italic">Payment will be processed once cards are confirmed by buyers.</p>
          </div>
        </div>
        ...
        <div className="pt-4 md:pt-6 border-t border-border space-y-4">
          <div className="flex gap-3">
            <Button
              ...
              variant="outline"
              size="sm"
              className="flex-1 border-border text-muted-foreground hover:bg-muted h-10 md:h-11 text-sm md:text-base"
            >
              Back
            </Button>
            <Button
              ...
              size="sm"
              className="flex-2 bg-primary hover:bg-primary/90 text-primary-foreground h-10 md:h-11 text-sm md:text-base font-bold shadow-lg shadow-primary/20"
            >
      ...
      <Card className="md:col-span-8 border-border bg-card/50 backdrop-blur-sm p-4 md:p-6 flex flex-col min-h-100 md:min-h-125">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <Label className="text-muted-foreground text-sm md:text-sm font-semibold uppercase tracking-wider">Verification Items</Label>
          ...
          {giftcards.map((card, idx) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center text-sm font-black text-primary">
                    {idx + 1}
                  </div>
                  <span className="text-xl md:text-2xl font-black text-foreground">${card.amount}</span>
                </div>
                <Badge className="bg-primary/20 text-primary hover:bg-primary/20 border-none text-sm px-1.5 h-4">Pending</Badge>
              </div>
