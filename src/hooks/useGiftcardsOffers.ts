...
export const useGiftcardsOffers = () => {
  const { isLoading, setIsLoading, offers, setOffers } = offersStore(
    (state) => state,
  );
  ...
  const getOffers = async (filters: { value: string; type: string }[]) => {
    try {
      ...
      const filteredOffers = offers.filter((offer) => {
        const countryOptions = filters.filter(
          (offer) => offer.type === "country",
        );
        const cardNameOptions = filters.filter(
          (offer) => offer.type === "brand",
        );

        const matchesCountry =
          countryOptions.length === 0 ||
          countryOptions.some((option) => offer.countryCode === option.value);

        const matchesCardName =
          cardNameOptions.length === 0 ||
          cardNameOptions.some((option) => offer.storeName === option.value);
