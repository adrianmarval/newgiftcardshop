...
import { adminCancelOrderInputSchema, adminCancelOrderOutputSchema } from '@/types/domain/admin';

export const adminCancelOrder = adminActionClient
  .inputSchema(adminCancelOrderInputSchema)
  .outputSchema(adminCancelOrderOutputSchema)
  ...
  .action(async ({ ctx }) => {
    await prisma.order.update({
      ...
      data: {
        ...
        giftcards: {
          updateMany: {
            where: { id: { in: ctx.order.giftcards.map((g) => g.id) } },
