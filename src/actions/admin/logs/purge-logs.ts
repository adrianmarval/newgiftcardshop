...
import { purgeLogsInputSchema, purgeLogsOutputSchema } from './schemas';
...
export const purgeLogs = adminActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    try {
      ...
      const where =
        olderThanDays > 0 ? { timestamp: { lt: new Date(Date.now() - olderThanDays * 24 * 60 * 60 * 1000) } } : {};
      ...
      logger.action(
        'batch',
        'purge-logs',
        `Admin purgó ${result.count} logs${olderThanDays > 0 ? ` mayores a ${olderThanDays} días` : ' (todos)'}`,
        {
