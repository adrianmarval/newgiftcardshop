...
- Tiers: giftcards tienen `escalationTier` que baja con el tiempo (cron en server.ts — tick FIJO 1min con config re-leída en cada tick; NUNCA derivar el intervalo del setInterval del setting `escalation_duration_minutes`, setInterval congela su intervalo al crearse y el cambio desde el panel no tomaría efecto sin reiniciar) — buyer solo puede tomar cards con tier ≤ su buyRate. **Schedule ANCLADO**: al dropear, `tierStartedAt` avanza al VENCIMIENTO del ciclo (`+1 ciclo`; `+3 ciclos` en la rama minTier-timeout), NUNCA a `now()` — resetear al tick acumulaba el atraso del tick (≤60s) en cada ciclo (con duration=1min la espera real llegaba a ~2x la estimada); anclado, un cron atrasado se pone al día solo porque la card sigue vencida el próximo tick. El estimador (`estimateTimeToAccess`) clampa el display a mínimo `~1 min` (ceil(0) mostraba "~0 min" durante la ventana vencida-no-procesada).
...
7. **Gate de seguridad para revelar códigos (anti-hurto)**: Los claim codes solo salen del servidor/bot si la orden NO tiene cards sin confirmar (`isConfirmed=false` ⇒ códigos aún aplicables y robables) O el buyer tiene un unlock vigente (`securityUnlockedUntil`, 10 min, cross-canal web+bot). El gate vive en el REVEAL, no en la creación de la orden (cero fricción en el flujo de compra). Es OBLIGATORIO al primer reveal: buyer sin PIN ni passkey debe crear un PIN inline antes de ver códigos. Puntos gateados: `getOrderCards` (redeem step), `getOrderById` (detalle/resume), `listOrders` (masked `••••••••` + flag `codesLocked`, solo scope buyer — admin nunca se enmascara), reveal post-creación y `order_detail` del buyer-bot. Web prioriza passkey (si hay + WebAuthn soportado) con fallback a PIN; Telegram solo PIN (no hay WebAuthn en el bot). **El unlock por passkey exige sesión FRESCA (<60s)**: `unlockWithPasskey` rechaza sesiones viejas leyendo `ctx.auth.session.createdAt` — la ceremonia `verify-authentication` siempre crea una sesión nueva (verificado en `@better-auth/passkey`: `createSession` + `setSessionCookie` incondicionales), así que una cookie robada (el escenario anti-hurto para el que existe el gate) no puede auto-otorgarse el unlock. **INVARIANTE — `customSession` DEBE retornar `{ user, session }`**: el plugin reemplaza el response de `getSession` con EXACTAMENTE lo que retorna su callback (verificado en `better-auth/plugins/custom-session`: `ctx.json(fnResult)`). Si el callback omite `session`, `getSession()` devuelve `{ user }` sin session en TODA la app y `ctx.auth.session.createdAt` crashea con `Cannot read properties of undefined` — bug real que dejó el unlock por passkey 100% roto (sept 2026). El tipo `Session` (`types/auth/session.ts`) declara el shape completo de session (`token`, `userId`, `createdAt`, `updatedAt`...) para que los casts `as Session` sigan compilando. Servicio compartido: `lib/services/security/security-pin.service.ts`. UI web: `components/buy/security/unlock-gate.tsx` + sección en perfil. Bot: `bot/buyer-bot/handlers/security-handler.ts` (wizard steps `awaitingSecurityPin`/`awaitingPinSetup*`/`awaitingPinReset*`/`awaitingPinChange*`, borra los mensajes con PIN/OTP vía `deleteUserInput`). Recuperación en ambos canales: OTP por email (`PinResetOtp` + template `pin-reset-otp.tsx`; `requestPinReset` NO swallowa errores a diferencia de `sendOtpEmail`). Lockout: 5 intentos fallidos → `pinLocked`, solo se recupera por email OTP.
...
- **`router.refresh()` está PROHIBIDO en TODA la app** (no solo en el SSE provider): cualquier refresh que cae con una navegación en vuelo la aborta y la URL revierte. El feedback post-mutación es `queryClient.invalidateQueries` de la query key de la vista. Las actions de MUTACIÓN siguen siendo server actions (su integración con el router es deseable); su éxito se refleja invalidando queries, nunca refrescando la ruta.

### Lecturas client-side via route handlers (`/api/query/[name]`)

**INVARIANTE ARQUITECTÓNICO — las lecturas de React Query NO usan server actions.** Una server action invocada imperativamente (un queryFn) se procesa en la cola de transiciones del App Router; si su reply aterriza con una navegación en vuelo, el router DESCARTA la navegación y la URL revierte al último commit. Verificado empíricamente (sept 2026, dev tunnel + Brave/Chrome): action POST en vuelo + click a los 100ms = ~60% de navs abortadas; fetch GET plano en las mismas condiciones = 0%. Como el SSE invalida queries constantemente, SIEMPRE había refetches en vuelo — la race era inevitable y era la causa del síntoma "voy a una ruta y se regresa sola". Bonus: la URL GET es estable entre deploys (adiós `Failed to find Server Action` 404s en tabs abiertas durante un redeploy).

- **Endpoint**: `src/app/api/query/[name]/route.ts` — GET catch-all con `QUERY_REGISTRY` (name → `{ roles, schema?, run(input, userId) }`). Los nombres matchean los query keys. Auth: `withRole(roles, handler)` de `src/lib/auth/route-guard.ts` — espejo de los action clients (401/403 JSON en vez de `unauthorized()`). Validación de input: los MISMOS zod schemas de las actions (`actions/*/schemas.ts`).
- **Por qué services y no llamar la action desde el route handler**: `next-safe-action/adapter-better-auth` importa `next/navigation.js` (con sufijo), cuyo grafo resuelve a un vendored `app-router-context` inexistente en el app-route module (MODULE_UNPARSABLE en Next 16). Por eso la lógica vive en `src/lib/services/` y TANTO las actions (primer paint server-side) COMO el route handler (refetch client-side) la comparten — una sola fuente de verdad.
- **Cliente**: `apiQuery<T>(name, input?)` (`src/lib/utils/api-query.ts`, barrel `@/lib/utils`) — GET con `?input=<json>`, desenvuelve `{ data }`, lanza en serverError/validation/HTTP error. Fechas: `serializeDates`/`deserializeDates` (`lib/utils/json-payload.ts`) con marker `{ $date: iso }` — JSON colapsa Date a string indistinguible; Flight las soportaba nativas.
- **Cubiertas**: las 8 listas, `unread-counts`, `notifications-page`, `live-availability`, dashboards (buyer/seller stats + recent, admin binance/platform/profit/inventory/stock-aging), `tours-seen` (topbar mount), `admin-telegram-photo` (UserBadge mount — antes disparaba UN POST de action POR badge renderizado).
- **Quedan como actions (deuda menor, user-triggered en dialogs)**: `getUserRates`, `getBatchImages`, `checkCodes`, `getOrderById` del wizard. Si alguna pasa a llamarse en mount/polling, migrarla al registry.
- **Para agregar una lectura client-side nueva**: service en `lib/services/` + entrada en `QUERY_REGISTRY` (con roles y schema) + `apiQuery` en el queryFn. NUNCA una server action desde un queryFn/useEffect.
