...
9. **Digest de stock anti-saturación (`STOCK_AVAILABLE`)**: In-app es canal pull → TODAS las notificaciones de stock se persisten al instante (el dispatcher persiste antes de encolar). Telegram/Push son canales interruptivos → `STOCK_AVAILABLE` NO se envía al publicar cada lote: el dispatcher llama `enqueueStockDigest(userId, brandCountryId)` (upsert en `StockDigestQueue`, `eventCount` incrementa, `firstEventAt` NO se toca — ventana fija) y retorna. Un sweep cada 5min (`server.ts` → `sweepStockDigests`, `lib/notifications/stock-digest.service.ts`) envía UN resumen por (buyer, brandCountry) cuando vence la ventana de la marca: `BrandCountry.stockDigestIntervalMinutes ?? stock_digest_interval_minutes` (setting global, default 30min, grupo notifications en admin config; el intervalo por marca se edita en el form de brand-country del admin — `updateBrandCountryLimits`). Máximo 1 mensaje por marca cada X min. El sweep re-verifica todo en fresco al enviar: canales activos (`telegramEnabled`/`pushEnabled`), suscripción a la marca, tasa asignada, y stock accesible REAL recomputado (agotado ⇒ silencio). Claim atómico por fila (`updateMany` + `claimedAt`, mismo patrón que telegram-topics — multi-instancia seguro); si el envío lanza, el claim se libera y reintenta el próximo ciclo; los descartes (canal off/unsubscribe/sin stock) se borran en silencio. `TIER_DROP_ACCESS` es **in-app only** (hardcoded en `IN_APP_ONLY_TYPES` del dispatcher — nunca sale por Telegram/Push; el buyer ya fue avisado por STOCK_AVAILABLE/digest cuando el stock llegó, un tier drop es info incremental que no amerita interrumpir). El resto de tipos (pagos, órdenes, batches) NO pasan por digest.
10. **Recordatorio de stock varado (`STOCK_REMINDER`)**: El digest cubre llegadas NUEVAS; el reminder cubre lo contrario — stock que nadie compra (mercado veloz: 24h de espera es inaceptable, los sellers valoran rotación rápida). Un sweep en el mismo tick de 5min del digest (`server.ts` → `sweepStockReminders`, `lib/notifications/stock-reminder.service.ts`) recorre marcas activas y, por cada buyer con tasa, envía UN recordatorio si TODO su stock accesible es más viejo que el intervalo de la marca: `BrandCountry.stockReminderIntervalMinutes ?? stock_reminder_interval_minutes` (setting global, default 60min, min 15, grupo notifications; el intervalo por marca se edita en el form de brand-country — `updateBrandCountryLimits`). **Un knob, dos funciones**: el intervalo es a la vez el umbral de "varado" y el cooldown por buyer. **Regla anti-ruido**: si el buyer tiene CUALQUIER tarjeta accesible más nueva que el intervalo → skip (ya fue nudgeado por `STOCK_AVAILABLE`/digest); el reminder solo dispara con el stock accesible completamente quieto. Cooldown en `StockReminderState` (la fila ES el dedup; upsert con `lastSentAt` epoch + claim atómico `updateMany` guardado — multi-instancia seguro); si el dispatch lanza, el claim se libera a epoch y reintenta el próximo ciclo. Envío via `notificationDispatcher.dispatch()` (persiste in-app + respeta telegram/push prefs); el filtro de suscripciones se aplica en el sweep (el dispatcher NO lo aplica). Complemento read-only: **Stock Aging report** en el admin dashboard (`actions/admin/stats/get-stock-aging-report.ts` + `components/admin/stock-aging-table.tsx`) — buckets de antigüedad `<1h/1-6h/6-24h/1-3d/>3d` por brand-country, ordenado por la tarjeta más vieja primero, para tunear escalación/recordatorios con criterio.
...
La app es realtime via **invalidación dirigida**, NO via payloads: el socket NUNCA transporta data de dominio (cero superficie de fuga de claim codes/data de otro rol), solo señales `{ keys: RealtimeKey[] }`. La fuente de verdad sigue siendo el servidor (RSC + Prisma + security gates); el cliente reacciona con `router.refresh()` dirigido.
...
- **Endpoint**: `src/app/api/realtime/route.ts` (SSE, `runtime=nodejs`, `force-dynamic`). Auth via cookie better-auth (`auth.api.getSession` directo — NO `getSession()` de authorization.ts, que hace `unauthorized()` y rompería el stream; sin sesión → 401). Filtrado server-side por `userId` + `role`. Heartbeat `: ping` cada 25s (proxies cierran idle), coalescing de 300ms por conexión (ráfagas → UN frame con la unión de keys), cleanup en `request.signal.abort`.
- **Cliente**: `src/providers/realtime-provider.tsx` (montado en `dashboard-layout.tsx`, reemplaza al viejo `AutoRefreshProvider`). `ROUTE_KEYS` mapea prefijo de ruta → keys que consume; un evento solo dispara `router.refresh()` si intersecta. Throttle 1s anti-ráfagas, pausa con tab oculto + resync al volver. **Conexión SSE ESTABLE por mount**: el EventSource NO se recrea al navegar — las keys de la ruta viven en un ref que `onmessage` lee en vivo (recrear por pathname generaba churn de suscripciones al bus y ruido ERR "stream canceled" en cloudflared/proxies). **Safety nets**: refresh incondicional en `onopen` tras reconexión (frames perdidos) y poll lento SIEMPRE activo (5min, solo tab visible) para convergencia eventual ante emit points que se hayan escapado.
- **Keys**: `notifications | orders | batches | availability | payments | stats | users | catalog | settings`.
- **Emit points**: `NotificationDispatcher.dispatch` (tras persistir → `['notifications']`, cubre TODOS los tipos), `order-lifecycle.service` (cancel/confirmUsage/complete → buyer `['orders','stats']`, sellers `['batches','stats']`, admin `['orders']`/`['orders','payments']`), `create-order.ts` (web) + `buy-handler.ts` (bot) (→ buyer `['orders','stats']`, rol BUYER `['availability']`, admin `['orders']`), `publish.service.ts` (→ seller `['batches','stats']`, BUYER `['availability']`, ADMIN `['batches']`), `escalation.ts` tier drops (→ BUYER `['availability']`), `seller-payout.service.ts` / `admin-withdrawal.service.ts` (payout enviado/revertido/sync resolved/failed → seller `['batches','stats']`, ADMIN `['payments']`), `stock-digest.service.ts` (digest entregado → `['availability']`), sweep auto-cancel en `server.ts` (→ sellers `['batches','stats']`, ADMIN `['batches']`).
- **Detección vs propagación**: realtime arregla la propagación servidor→cliente (<1s); la frecuencia de DETECCIÓN de lo que depende de cron (tier drops, sync Binance, digests) sigue siendo la del cron.
- Los `router.refresh()` post-mutación existentes (admin views, registry-card, settings) SE MANTIENEN — son feedback inmediato de acciones propias.
