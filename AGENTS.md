...
- **Cliente**: `src/providers/realtime-provider.tsx` (montado en `dashboard-layout.tsx`, reemplaza al viejo `AutoRefreshProvider`). `ROUTE_KEYS` mapea prefijo de ruta → keys que consume; un evento solo dispara `router.refresh()` si intersecta. Admin usa keys GRANULARES por subruta (orders→`['orders','notifications']`, batches→`['batches','notifications']`, etc.) — consumir todas las keys en todo el portal disparaba un refresh por cualquier evento del marketplace. Throttle 1s anti-ráfagas, pausa con tab oculto + resync al volver. **INVARIANTE — TODO `router.refresh()` de fondo va dentro de `startTransition`**: un refresh que cae mientras hay una navegación del usuario en vuelo (Link click, paginación nuqs con `shallow:false` = `router.replace` completo) la ABORTA y la URL revierte al último estado commiteado (bug conocido del App Router — síntoma: paginación veloz que "regresa" a páginas anteriores, o soft-nav entre secciones que rebota a la página de origen). En transition el refresh es baja prioridad y la navegación del usuario lo preempta. NUNCA llamar `router.refresh()` directo en el provider. **Conexión SSE ESTABLE por mount**: el EventSource NO se recrea al navegar — las keys de la ruta viven en un ref que `onmessage` lee en vivo (recrear por pathname generaba churn de suscripciones al bus y ruido ERR "stream canceled" en cloudflared/proxies). **Safety nets**: refresh incondicional en `onopen` tras reconexión (frames perdidos); **watchdog anti half-open** (tunnels/proxies como cloudflared pueden dejar de entregar frames SIN cerrar el TCP — `onerror` nunca dispara y EventSource jamás reconecta solo): el cliente trackea `lastFrameAt` en cada frame (data o ping) y si pasan >75s sin frames (~3 heartbeats) con tab visible, cierra y reconecta manualmente (onopen resynca); poll lento SIEMPRE activo (5min, solo tab visible) para convergencia eventual ante emit points que se hayan escapado.
